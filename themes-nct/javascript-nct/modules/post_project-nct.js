...
$(function(){
	...
	$('[name="startDate"]').datepicker({
	   ...
	   $('[name=biddingDeadline]').datepicker('setEndDate', endDate);
	}).on('clearDate', function (selected) {
	   $('[name=biddingDeadline]').datepicker('setEndDate', null);
	});*/

});

/*$(document).on('change','.days-number',function(e){
	$('.amount #amountInDefaultCurr').html($(this).val()*featuredProjPrice);
	//to show calculated featured amount
	submitValueHandler(siteUrl,'action=method&method=currencyHelper&amount='+$('.amount #amountInDefaultCurr').html()+'&to='+fromCurr,'Please wait..',function(data){
        $('#calculatedFeaturedPrice').html(data.html); 
    });
});*/
