<?php
	...
	$mainObject = new City($id,$searchArray,$action);
...
*/		
	...
	/*if(isset($_POST['country'])) {
		echo $fields->selectBox(array(
			"label" =>"".MEND_SIGN."State:",
			"onlyField"=>false,
			"name"=>"state",
			"defaultValue"=>true,
			"allow_null"=>1,
			...
			"intoDB"=>array("val"=>true,
				   ...
				   "custom"=>true,
					"fields"=>"id,stateName",
					"where"=>"status='a' AND country_id=".$_POST['country']." ",
					"orderBy"=>"stateName",
					"valField"=>"id",
					"dispField"=>"stateName"
			)
	...
	{
		extract($_POST);
		echo json_encode(array('states'=>$mainObject->getState(0,$cid)));
		exit;
	...
	{
		extract($_POST);
		echo json_encode(array('cities'=>$mainObject->getCity(0,$sid)));
		exit;
