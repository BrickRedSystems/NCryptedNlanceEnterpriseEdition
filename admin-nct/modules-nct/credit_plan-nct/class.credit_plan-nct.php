<?php
class Credit_plan extends Home{
	public $data = array();
...
$this->db = $db;
		...
		if($this->id>0){
			...
			$fetchRes = $this->db->pdoQuery("SELECT m.* FROM tbl_credit_plans AS m
	...
	public function viewForm(){		
		$content = $this->displayBox(array("label"=>"Plan name&nbsp;:","value"=>$this->planName))
				.$this->displayBox(array("label"=>"Plan Price&nbsp;:","value"=>CURRENCY_SYMBOL.$this->price))
	...
	public function dataGrid() {
		...
		$whrCond = '';			
        if (isset($chr) && $chr != '') {
            $whereCond .= "  WHERE (m.planName LIKE '%" . $chr . "%' OR DATE_FORMAT(m.createdDate, '" . MYSQL_DATE_FORMAT . "') LIKE '%" . $chr . "%')";
        }		
        ...
        $query = "SELECT m.* FROM tbl_credit_plans AS m " . $whereCond . " ORDER BY " . $sorting;			
		...
		foreach($qrySel as $fetchRes) {
			...
			$final_array = array(
				...
				ucfirst(strtolower(stripslashes($fetchRes["planName"]))) ,
