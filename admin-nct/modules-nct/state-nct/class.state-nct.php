...
class State extends Home {
	...
	public function __construct($id=0, $searchArray=array(), $type='') {		
		...
		if($this->id>0){
			...
			$fetchRes = $qrySel;
			$this->data['id'] = $this->id = $fetchRes['StateID'];
			$this->data['country_id'] = $this->country_id = $fetchRes['CountryID'];
			$this->data['stateName'] = $this->stateName = $fetchRes['stateName'];
			$this->data['status'] = $this->status = $fetchRes['isActive'];
		}else{
	...
	public function getForm() {
		$content = $section_opt = '';
			...
			$opt_content = new MainTemplater(DIR_ADMIN_TMPL."select_option-nct.tpl.php");
			$opt_content = $opt_content->parse();
			$qrySel = $this->db->select("tbl_country", array("CountryId","countryName","isActive"))->results();
			foreach($qrySel as $opt){
				$selected = $this->data['country_id'] == $opt['CountryId'] ? 'selected' : '';
				if($opt['isActive']=='y'){
				$fields_replace = array($opt['CountryId'],$opt['countryName'],$selected);
				$section_opt .=str_replace(array("%VALUE%","%DISPLAY_VALUE%","%SELECTED%"),$fields_replace,$opt_content);
				}
			}
			...
			$static_a=($this->data['status'] == 'y' ? 'checked':'');
			$static_d=($this->data['status'] != 'y' ? 'checked':'');
	
			$fields = array("%SECTION_OPT%","%STATE_NAME%","%STATIC_A%","%STATIC_D%","%TYPE%","%ID%");
	
			$fields_replace = array($section_opt,$this->data['stateName'],$static_a,$static_d,$this->type,$this->id);
	...
	public function dataGrid() {
		...
		foreach($qrySel as $fetchRes) {
			...
			$final_array =  array($fetchRes["stateName"],$fetchRes["countryName"]);
