<?php
class ReportProject extends Home {
	...
	public function viewForm() {								    	
        $content = $this->displayBox(array("label" => "Title :", "value" => $this->title)) 
                ...
                . $this->displayBox(array("label" => "Posted on :", "value" => $this->createdDate));
        return $content;
    }
	...
	public function getForm()
	{
        ...
        $replace = array(
            ...
            "%USER_NAME%" => ucwords($this->userName),
            "%EMAIL%" => strtolower($this->email),
            "%PROJECTID%" => $this->projectId,
            "%DATE%" => $this->createdDate,
            "%UTYPE%" => ($this->userType == 'c')?'Customer':'Provider',
	...
	public function dataGrid() {
        ...
        if (isset($chr) && $chr != '') {
            $whereCond .= "  WHERE (u.`firstName` LIKE '%" . $chr . "%' OR u.`lastName` LIKE '%" . $chr . "%' OR p.title LIKE '%" . $chr . "%' OR DATE_FORMAT(r.createdDate, '" . MYSQL_DATE_FORMAT . "') LIKE '%" . $chr . "%')";
        }		
		...
		foreach($qrySel as $fetchRes) {
			...
			$id = $fetchRes['id'];
			$operation = NULL;
			$operation .=(in_array('edit',$this->Permission))?'&nbsp;&nbsp;'.$this->operation(array("href"=>SITE_ADM_MOD.$this->module."/ajax.".$this->module.".php?action=edit&id=".$id,"class"=>"btn default btn-xs black btnEdit","value"=>'<i class="fa fa-edit"></i>&nbsp;Reply')):NULL;
			...
			$final_array = array(		
				...
				stripslashes($fetchRes["emailStatus"]=='y'?'Email Sent':'Pending') ,						
				$operation				
			);
