<?php
	...
	$mainObject = new Projects($module,$id,$searchArray,$action);
	if($action == "delete"  && in_array('delete',$Permission)) {	
		$aWhere=array("id"=>$id);
		$db->delete($table,$aWhere);
		unlink(DIR_UPD.'projects/'.$image);
		unlink(DIR_UPD.'projects/th1_'.$image);
	...
	if($action == "getSubcategory" && isset($_REQUEST['cid']) && $_REQUEST['cid']>0) {
		$html = $mainObject->get_subcats($_REQUEST['cid']);
		echo json_encode(array('type'=>'success','subcategory'=>$html));
		exit;
	}
